...
**一个节点就是一个运行在集群模式下的Redis服务器**，Redis服务器在启动时会根据`cluster-enabled`配置选项是否为yes来决定是否开启服务器的集群模式:![img](https://camo.githubusercontent.com/9470157e91b21a4ecfa9df49d7e8d9f263abae6fcef6b45d2faa49045f749537/68747470733a2f2f756b2d313235393535353837302e636f732e65752d6672616e6b667572742e6d7971636c6f75642e636f6d2f32303230303131313131323535382e706e67)
...
#### redisClient 与 clusterLink 结构比较

> redisClient 结构与 clusterLink 结构都有自己的套接字描述符和输入、输出缓冲区，这两个结构的却别在于，redisClient中的套接字和缓冲区是用于连接客户端的，而clusterLink中的套接字和缓冲区是则是用于连接节点的。

假设现在有三个独立的节点127.0.0.1:7000、127.0.0.1:7001、127.0.0.1:7002，将他们创立为集群后，节点7000的数据结构如下：![img](https://camo.githubusercontent.com/3e878d3e16b482f58f9cdc731ca5c07deea4f8e68da8f78e1d14e3e594bb2ace/68747470733a2f2f756b2d313235393535353837302e636f732e65752d6672616e6b667572742e6d7971636c6f75642e636f6d2f32303230303131313131333934312e706e67)
...
![image-20210407163741300](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407163748.png)
...
下图表示这个节点负责处理槽0至7![image-20210407163924210](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407163924.png)

因为数组自带索引，所以取出某个槽是否使用的时间复杂的为O(1)
...
- 通过`clusterState`保存的数组，可以以O(1)的时间取得结果。

### 17.2.4 CLUSTER ADDSLOTS 命令的实现

CLUSTER ADDSLOTS 命令接受一个或多个槽作为参数，并将所有输入的槽指派给接受该命令的结点负责
...
![image-20210407165048220](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407165048.png)
...
![image-20210407165228704](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407165228.png)
...
其中slot为键所在的槽，而ip和port则是负责处理槽slot的节点的IP地址和端口号，实际上就是套接字的转换，换一个套接字来发送命令。
...
节点与单击服务器在数据库方面的一个区别是，节点只能使用0号数据库，而单击Redis服务器则没有这一限制

除了将键值对保存在数据库里面之外，节点还会用`clusterState`结构中的`slots_to_keys`**跳跃表**来保存槽和键之间的关系:
...
![image-20210407165923328](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407165923.png)
...
![image-20210407170011985](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407170012.png)

如果重新分片涉及多个槽，那么redis-trib将对每个给定的槽分别执行上面给出的步骤。

![image-20210407170018365](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407170018.png)
...
节点7003的clusterState.importing_slots_from数组将变成下图所示：

![image-20210407170223378](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407170223.png)
...
![image-20210407170444088](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407170444.png)

接到ASK错误的客户端会根据错误提供的IP地址和端口号，**转向至正在导入槽的目标节点，然后首先向目标节点发送一个ASKING命令**，之后再重新发送原本想要执行的命令。

![image-20210407170506013](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407170506.png)

### 17.5.4 ASKING命令

ASKING命令的目的就是打开REDIS_ASKING标识，而且**是一次性的打开**，意味着使用完后会被关闭。![image-20210407170527418](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407170527.png)
...
![image-20210407170718259](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407170718.png)
...
接收到PUBLISH命令的节点不仅会向channel频道发送消息message，它还会**向集群广播一条PUBLISH消息，**所有接收到这条PUBLISH消息的节点都会向channel频道发送message消息

也就是说，向集群发送`PUBLISH <channel> <message>`，会导致集群所有节点都向channel发送message消息。

![image-20210407171022857](https://happychan.oss-cn-shenzhen.aliyuncs.com/img/pic/20210407171022.png)